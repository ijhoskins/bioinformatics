...
usage() {
cat << EOF  
Usage: ./get_region_bed.sh [-h] -i TRANSCRIPT_IDs -t ENSEMBL_GTF
...
fgrep -f "$TRX_ID_FILE" "$GTF" | awk '{if($3=="five_prime_utr" || $3=="three_prime_utr" || $3=="CDS") {print}}' "$FILT_GTF" - > filt.gtf
